...
try:
    AddonVersion = control.addon('plugin.video.fuzzybritches_v2').getAddonInfo('version')
    ModuleVersion = control.addon('script.module.fuzzybritches_v2').getAddonInfo('version')
...
def syncTraktLibrary():
    control.execute('RunPlugin(plugin://%s)' % 'plugin.video.fuzzybritches_v2/?action=tvshowsToLibrarySilent&url=traktcollection')
    control.execute('RunPlugin(plugin://%s)' % 'plugin.video.fuzzybritches_v2/?action=moviesToLibrarySilent&url=traktcollection')
